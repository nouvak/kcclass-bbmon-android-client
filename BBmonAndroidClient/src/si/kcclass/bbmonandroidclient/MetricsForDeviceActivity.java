...
public class MetricsForDeviceActivity extends ListActivity {
	...
	protected List<Metric> doInBackground(String... params) {
	    ...
	    try {
		data = restClient.invokeService("/devices/getMetrics",
	...
	protected void onPostExecute(List<Metric> data) {
	    // mMetricData = data;
	    /*
	     * List<String> listNames = new ArrayList<String>(); for (Metric
	     * device : data) { listNames.add(device.getName()); }
	     * 
		...
		mMetricAdapter.notifyDataSetChanged();
		for (int i = 0; i < data.size(); i++)
		    mMetricAdapter.add(data.get(i));
	    }
	    mMetricAdapter.notifyDataSetChanged();
	}
    ...
    private static final String LOG_TAG = MetricsForDeviceActivity.class
	    .getName();
	...
	mMetricData = new ArrayList<Metric>();
	mMetricAdapter = new MetricAdapter(this, R.layout.row_metric,
		mMetricData);
	setListAdapter(mMetricAdapter);
	loadMetric();
	updateTitle();
	/*
	 ...
	 * int position, long id) { itemClicked(position); }
	 * 
	 * });
	 */
    }
